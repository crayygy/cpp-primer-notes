...
```
  ...
  让我们来详细解析一下这段C++代码，便于理解。
  
  首先`#include<iostream>`在程序中包含一个标准的输入输出流，这样才可以使用后面的cout,cin等等；
  
  然后是一个`int main(){ }`函数，每一个C++工程都需要 *有且只有* 一个main()函数，这样代码才可以正确运行；
  
  在main函数的代码块里，我们开始了第一行`cout`, 输出流，表示向外输出一个语句，即将
`Enter two numbers: `打印在窗口；

  下一行申请了两个变量 `v1` 和 `v2`，要注意，`变量必须先申明，再调用，函数也一样(在后面会详细讲解)`；
  
  下一行使用标准输入语句`cin`来将输入的字符依次赋值到`v1` `v2`；
  
  再后面则是将`v1` `v2`的值相加，并且将输出的值打印在窗口。
  
  最后别忘了`return 0;`。
  
  这段代码不是很复杂，尝试自己修改代码，让程序输出你想要的结果，多尝试几次，总结一些规律。

  其实上面的代码也可以写成这样，稍微省事一点：
